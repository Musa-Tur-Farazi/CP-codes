struct veb{
    ...
    veb(int size){
        ...
        summary = new veb(no_clusters);
        clusters = vector<veb*>(no_clusters, NULL);
    ...
    */
    void insert(veb* root,int key){
        if (root->min == -1) {
            ...
            if (key < root->min) { swap(root->min, key); }
            if (key > root->max) { root->max = key; }
    ...
    inline void insert(int x){ insert(this,x); }
    inline int successor(int x){ return successor(this,x); }
    inline void del(int x){ del(this, x); }
    inline int get_max(){ return this->max; }
    inline int get_min(){ return this->min; }
};
